...
class _MyHomePageState extends State<MyHomePage> {
  ...
  Future<List<Comments>> fetchComments()  async {
    ...
    if (response.statusCode == 200) {
      var getCommentsData = json.decode(response.body) as List;
      var listComments = getCommentsData.map((i) => Comments.fromJson(i)).toList();
      return listComments;
    } else {
  ...
  Future<List<Products>> fetchproducts()  async {
    ...
    if (response.statusCode == 200) {
      var getCommentsData = json.decode(response.body.toString());
      var res=ResponseMain.fromJson(getCommentsData);
      var listproducts =res.products ;
      return listproducts;
    } else {
      throw Exception('Failed to load comments');
    }
